...
class Trie {
 public:
  ...
  void Insert(std::string& string, const int& pattern) {
    int index = 0;
  ...
  void PrintEntries(const int& i, int index) {
    ...
    while ((index != 0) && (index != -1)) {
      result_index[states[index].pattern_index].push_back(i - states[index].pattern_length + 2);
      index = states[index].exit_link;
    }
  }
};
...
void AhoCorasik(const std::string& string, const int& patterns_size) {
  ...
  for (int i = 0; i < total_patterns; ++i) {
    std::cout << trie.result_index[i].size() << ' ';
    int quantity_curr_patter = static_cast<int>(trie.result_index[i].size());

    for (int j = 0; j < quantity_curr_patter; ++j) {
      std::cout << trie.result_index[i][j] << ' ';
    }
